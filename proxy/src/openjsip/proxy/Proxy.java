...
public class Proxy extends UnicastRemoteObject implements SipListener, RemoteServiceInterface, Runnable
{
    ...
    /**
     * Method openjsip.proxy.plugins, such as REGISTER
     */
    ...
    public class QoSRequest implements Serializable {
        ...
        InetAddress destIP;
        int rate;
        boolean statusConnexion; //Deconnexion ou connexion
    }



    public QoSResponse requestToBB(QoSRequest qoSRequest){

        InetAddress adr;
        Socket socketToBB;

        ServerSocket listenSocket;
        ObjectInputStream fromBB;
        QoSResponse response = new QoSResponse(false); //Par défaut faux, si pas de reponse du BB

        try {

            //Notification au BB
            listenSocket = new ServerSocket(4000);
            socketToBB = new Socket(qoSRequest.destIP.getHostAddress(),qoSRequest.destPort);
            ObjectOutputStream out = new ObjectOutputStream(socketToBB.getOutputStream());
            out.writeObject(qoSRequest);
            out.flush();

            //Attente de la réponse du BB
            socketToBB = listenSocket.accept();
            fromBB = new ObjectInputStream(socketToBB.getInputStream());
            response = (QoSResponse) fromBB.readObject();
    ...
    {                                                                                                               
        ...
        /**
         * It's time to load method openjsip.proxy.plugins (REGISTER for example).
         */
        if (log.isInfoEnabled())
            log.info("Loading method openjsip.proxy.plugins...");
...
if(((CSeqHeader)response.getHeader(CSeqHeader.NAME)).getMethod().equals("INVITE")) {
                    	...
                    	for (Iterator it = sr.getHeaders(); it
								.hasNext();) {
							SIPHeader typy = (SIPHeader) it.next();
							System.out.println("------------CSeqHEADER-----------");
							System.out.println("HERE" + typy.getHeaderName());
							System.out.println("------------CSeqHEADER-----------");
							if(typy.getHeaderName().equals("Content-Type")) {
							System.out.println("------------CSeqHEADER-----------");
								System.out.println("HERE2" + typy.getHeaderValue());
							System.out.println("------------CSeqHEADER-----------");
								
								   //Save the SDP content in a String
								   String sdpContent = 
								             new String(sr.getRawContent());  

								   //Use the static method of SdpFactory to
								   //parse the content
								   try {
								   SdpFactory sdpf = SdpFactory.getInstance();
								   SessionDescription requestSDP = sdpf.createSessionDescription(sdpContent);
							System.out.println("------------CSeqHEADER-----------");
							System.out.println("HERE4"+requestSDP.toString());
							System.out.println("------------CSeqHEADER-----------");
								   } catch (Exception e) {
									   e.printStackTrace();
								   }
